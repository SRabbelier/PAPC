...
void seq_function(int n, int log_n)
{
	...
	for(h = 1; h <= log_n; h++)
	{
		int h_pow = pow(2, h);
		int size = n / h_pow;
		//#pragma omp parallel for schedule(static, 1) shared(B_,h_pow,size,h) private(j) num_threads(4)
		for(j = 1; j <= size; j++)
		{
			int prev, curr, next;
	...
	for(x = log_n; x >= 0; x--)
	{
		//#pragma omp parallel for schedule(static, 1) shared(B_,C_,S_,x,prev_size,size) private(i) num_threads(4)
		for(i = 0; i < size; i++) 
		{
